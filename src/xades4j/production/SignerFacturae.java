 ...
 * @author Luís
 */
class SignerFacturae implements XadesSigner {
	...
	private static int RANDOM_SIZE = 1000000;

	@Inject
	protected SignerFacturae(KeyingDataProvider keyingProvider, AlgorithmsProviderEx algorithmsProvider,
			BasicSignatureOptionsProvider basicSignatureOptionsProvider,
			SignedDataObjectsProcessor dataObjectDescsProcessor, SignaturePropertiesProvider signaturePropsProvider,
			DataObjectPropertiesProvider dataObjPropsProvider, PropertiesDataObjectsGenerator propsDataObjectsGenerator,
			SignedPropertiesMarshaller signedPropsMarshaller, UnsignedPropertiesMarshaller unsignedPropsMarshaller,
			AlgorithmsParametersMarshallingProvider algorithmsParametersMarshaller) {
		if (ObjectUtils.anyNull(keyingProvider, algorithmsProvider, signaturePropsProvider, dataObjPropsProvider,
				propsDataObjectsGenerator, signedPropsMarshaller, unsignedPropsMarshaller,
				algorithmsParametersMarshaller)) {
	...
	@Override
	public final XadesSignatureResult sign(SignedDataObjects signedDataObjects, Node referenceNode,
		...
		String signatureId = String.format("Signature%s", random.nextInt(RANDOM_SIZE));
		String signedPropsId = String.format("%s-SignedProperties%s", signatureId, random.nextInt(RANDOM_SIZE));
		...
		XMLSignature signature = createSignature(signatureDocument, signedDataObjects.getBaseUri(),
		...
		Map<DataObjectDesc, Reference> referenceMappings = this.dataObjectDescsProcessor.process(signedDataObjects,
				signature);
		...
		Element qualifyingPropsElem = ElementProxy.createElementForFamily(signature.getDocument(),
		...
		// qualifyingPropsElem.setAttributeNS(Constants.NamespaceSpecNS,
		// "xmlns:xades141", QualifyingProperty.XADESV141_XMLNS);
		...
		QualifyingProperties qualifProps = qualifPropsProcessor.getQualifyingProperties(signedDataObjects, fsssp,
				fsusp);

		try {
			// The signature needs to be appended to the document from now on
			// because
			// property data generation may need to dereference same-document
			// data
			// object references.
			...
			PropertiesDataGenerationContext propsDataGenCtx = new PropertiesDataGenerationContext(
					signedDataObjects.getDataObjectsDescs(), referenceMappings, signatureDocument);
			// Generate the signed properties data objects. The data objects
			// structure
			// is verifier in the process.
			SigAndDataObjsPropertiesData signedPropsData = this.propsDataObjectsGenerator
					.generateSignedPropertiesData(qualifProps.getSignedProperties(), propsDataGenCtx);
			// Marshal the signed properties data to the QualifyingProperties
			// node.
			...
			try {
				signature.addDocument('#' + signedPropsId, null, digestAlgUri, null,
						QualifyingProperty.SIGNED_PROPS_TYPE_URI);
			} catch (XMLSignatureException ex) {
				// Seems to be thrown when the digest algorithm is not
				// supported. In
				// this case, if it wasn't thrown when processing the data
				// objects it
			...
			Element sigValueElem = DOMHelper.getFirstDescendant(signature.getElement(), Constants.SignatureSpecNS,
					Constants._TAG_SIGNATUREVALUE);
			DOMHelper.setIdAsXmlId(sigValueElem, String.format("SignatureValue%s", random.nextInt(RANDOM_SIZE)));
			...
			SigAndDataObjsPropertiesData unsignedPropsData = this.propsDataObjectsGenerator
					.generateUnsignedPropertiesData(qualifProps.getUnsignedProperties(), propsDataGenCtx);
			// Marshal the unsigned properties to the final QualifyingProperties
			// node.
	...
	private XMLSignature createSignature(Document signatureDocument, String baseUri, String signingKeyAlgorithm)
			throws XAdES4jXMLSigException, UnsupportedAlgorithmException {
		...
		Element canonAlgElem = createElementForAlgorithm(canonAlg, Constants._TAG_CANONICALIZATIONMETHOD,
				signatureDocument);
	...
	private Element createElementForAlgorithm(Algorithm algorithm, String elementName, Document signatureDocument)
		...
		List<Node> algorithmParams = this.algorithmsParametersMarshaller.marshalParameters(algorithm,
				signatureDocument);
	...
	/**
	 * Override in subclasses to collect the signature properties that are
	 * mandatory in the corresponding format.
	 */
